...
async function getMessageCount(telegramId) {
    try {
        const db = getDb();
        if (!db)
            return 0;
...
async function upsertProfile(telegramId, firstName, username) {
    try {
        const db = getDb();
        if (!db)
            return;
...
async function getAllCompletedCourses(telegramId) {
    try {
        const db = getDb();
        if (!db)
            return [];
...
function loadModuleContent(courseId, moduleId) {
    try {
        const p = (0, path_1.join)('content', 'courses', courseId, `${moduleId}.md`);
...
function getModuleTitle(_courseId, moduleId) {
    const titles = { 'intro': 'Introduction to AI', 'prompt-engineering': 'Prompt Engineering', 'vector-databases': 'Vector Databases', 'llm-integration': 'LLM Integration', 'building-apps': 'Building AI Apps', 'telegram-basics': 'Telegram Bot Basics', 'python-bots': 'Python Bots', 'ai-integration': 'AI Integration' };
    return titles[moduleId] || moduleId;
}
function getNextModule(courseId, currentModule) {
    const mods = COURSES[courseId]?.modules;
    if (!mods)
        return null;
    const idx = mods.indexOf(currentModule);
...
bot.use(async (ctx, next) => { console.log('📨 Received:', ctx.updateType); return next(); });
...
bot.command('learn', async (ctx) => {
    ...
    if (!args.length) {
        await ctx.reply('/learn <course> [module]');
        return;
    }
    const courseId = args[0];
    const course = COURSES[courseId];
    if (!course) {
        await ctx.reply('Not found. /courses');
        return;
    }
    if (!args[1]) {
    ...
    if (telegramId)
        await markModuleDone(telegramId, courseId, moduleId);
    ...
    if (next)
        await ctx.reply(`➡️ Next: /learn ${courseId} ${next}`);
    else
        await ctx.reply('🎉 Course Complete! 🏆');
});
bot.command('memory', async (ctx) => {
    ...
    for (const r of recent)
        m += `${r.role === 'user' ? '👤' : '🤖'} ${r.content.slice(0, 120)}\n`;
...
bot.command('progress', async (ctx) => {
    ...
    if (courses.length) {
        ...
        for (const c of courses) {
            const co = COURSES[c.course];
            m += `📚 ${co?.title || c.course}: ${c.completed}/${c.total}\n`;
        }
    }
    else
        m += '📚 No courses yet. /learn ai-engineering-101';
    await ctx.reply(m, { parse_mode: 'HTML' });
});
