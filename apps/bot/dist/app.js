...
function loadModuleContent(courseId, moduleId) {
    const path = (0, path_1.join)('content', 'courses', courseId, `${moduleId}.md`);
    try {
...
class BotService {
    ...
    webhookUrl;
    bot;
    constructor(ai, transcriber, webhookUrl) {
        this.ai = ai;
        ...
        this.bot.command('start', async (ctx) => {
            ...
            if (id)
                await upsertProfile(id, ctx.from?.first_name || 'Student', ctx.from?.username);
            await ctx.reply('👋 Welcome to <b>Getedil</b>! 🚀\n\n📚 /courses | 💼 /jobs | 📝 /memory | 📊 /progress\n\nJust send a message or voice note!', { parse_mode: 'HTML' });
        });
        ...
        this.bot.command('jobs', async (ctx) => {
            await ctx.reply('💼 <b>Ethiopian Tech Jobs</b>\n\n' +
                ...
                'Data Scientist - CBE\n' +
                'AI Trainer - Upwork/Fiverr', { parse_mode: 'HTML' });
        });
        // ============================================
        // Memory (REAL)
        // ============================================
